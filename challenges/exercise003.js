...
export function camelCaseWords(words) {
  ...
  return words.reduce(
    (output, word, index) => output + (index === 0 ? word : capitalize(word)),
    ""
  );
}
...
export function checkIngredients(menu, ingredient) {
  ...
  return (
    menu.filter((dish) => new Set(dish.ingredients).has(ingredient)).length > 0
  );
}
...
export function duplicateNumbers(arr1, arr2) {
  ...
  const commonValues = [...numbersInArr2].filter((value) =>
    numbersInArr1.has(value)
  );
