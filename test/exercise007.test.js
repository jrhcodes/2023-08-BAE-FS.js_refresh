...
describe("exercise007 sumDigits tests", () => {
  ...
  test("returns the sum of digits for integer values", () => {
    ...
    expect(sumDigits("1234.567890")).toBe(
      1 + 2 + 3 + 4 + 5 + 6 + 7 + 8 + 9 + 0
    );
    expect(sumDigits(-12345678.9)).toBe(1 + 2 + 3 + 4 + 5 + 6 + 7 + 8 + 9 + 0);
  });
});
...
describe("exercise007 createRange() tests", () => {
  test("when missing/invalid parameter - throws error", () => {
    ...
    expect(() => createRange(1, 2, -1)).toThrow(
      "step value will not increment/decrement from start to end"
    );
    expect(() => createRange(-1, -2, 1)).toThrow(
      "step value will not increment/decrement from start to end"
    );
    expect(() => createRange(-1, -2, 0)).toThrow(
      "step value will not increment/decrement from start to end"
    );
    expect(() => createRange(1, 2, 0)).toThrow(
      "step value will not increment/decrement from start to end"
    );
    expect(() => createRange(2, 3, 2)).toThrow(
      "step value will not increment/decrement from start to end"
    );
    expect(() => createRange(-2, -3, -2)).toThrow(
      "step value will not increment/decrement from start to end"
...
describe("exercise007 getScreentimeAlertList tests", () => {
  test("generates list of users overusing screen time given date", () => {
    const userData = [
      {
        ...
        screenTime: [
          ...
          {
            date: "2019-06-13",
            usage: { mapMyRun: 50, MORGOTH: 20, facebook: 40, safari: 16 },
          },
        ],
      },
      {
        ...
        screenTime: [
          {
            date: "2019-05-04",
            usage: { twitter: 40, instagram: 6, facebook: 61 },
          },
          {
            date: "2019-06-11",
            usage: { mapMyRun: 10, whatsApp: 10, facebook: 80, safari: 10 },
          },
          {
            date: "2019-06-13",
            usage: { mapMyRun: 30, whatsApp: 50, facebook: 20, safari: 16 },
          },
          {
            date: "2019-06-14",
            usage: { mapMyRun: 0, whatsApp: 99, facebook: 0, safari: 31 },
          },
          {
            date: "2019-06-30",
            usage: { mapMyRun: 33, whatsApp: 0, facebook: 33, safari: 33 },
          },
        ],
      },
      {
        ...
        screenTime: [
          {
            date: "2019-05-01",
            usage: { twitter: 34, instagram: 99, facebook: 40 },
          },
          {
            date: "2019-05-02",
            usage: { twitter: 56, instagram: 40, facebook: 31 },
          ...
          {
            date: "2019-05-04",
            usage: { twitter: 10, instagram: 76, facebook: 61 },
          },
          {
            date: "2019-06-11",
            usage: {
    ...
    expect(getScreentimeAlertList(userData, "2019-05-01")).toEqual([
      "jrhcodes",
    ]);
    expect(getScreentimeAlertList(userData, "2019-05-02")).toEqual([
      "beth_1234",
      "jrhcodes",
    ]);
    ...
    expect(getScreentimeAlertList(userData, "2019-06-11")).toEqual([
      "sam_j_1989",
      "jrhcodes",
    ]);
    expect(getScreentimeAlertList(userData, "2019-06-13")).toEqual([
      "beth_1234",
      "sam_j_1989",
    ]);
  ...
  test("throws an error when given missing/invalid parameters", () => {
    expect(() => getScreentimeAlertList(undefined, "")).toThrow(
      "users is required"
    );
